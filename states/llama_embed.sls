...
download_embed_model:
  cmd.run:
    - name: >-
        ...
        -o {{ model_path }}
        "https://huggingface.co/{{ embed.repo }}/resolve/main/{{ embed.file }}"
    - creates: {{ model_path }}
    - timeout: 1800
    - retry:
    ...
    - require:
      - file: llama_embed_models_dir

{{ service_with_unit('llama_embed', 'salt://units/llama-embed.service', template='jinja', context={'user': user, 'home': home, 'models_dir': models_dir, 'model_file': embed.file, 'port': port, 'context': embed.context, 'gpu_layers': embed.gpu_layers, 'pooling': embed.pooling}, requires=['cmd: download_embed_model', 'cmd: install_llama_cpp_vulkan'], onlyif='command -v llama-server') }}
